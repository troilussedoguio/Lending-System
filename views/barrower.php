					...
					<div class="table-main-div">
						<div class="table-header">
							<div class="d-flex justify-content-between row-gap-3">
                                <strong class="header-title text-secondary">Barrower Informations</strong>
                                <a href="add-new-barrower.php" class="btn btn-primary btn-sm">
									<i class="ri-add-line"></i>   
									<span class="d-none d-lg-inline-block">Add New Barrower</span>
								</a>
                            </div>
						</div>
							...
							<table id="Datatable-format" class="display responsive" width="100%">
								<thead>
									<tr>
										...
										<th class="min-phone-l">Phone Number</th>
								...
								<tbody>
									<?php 
										foreach($fetchBarrower as $b): 
										unset($b['password']); 
										?>
										
										<tr>
											<td><?= $b['id'] ?></td>
											<td class="text-nowrap"><?= htmlspecialchars($b['full_name']) ?></td>
											<td><?= htmlspecialchars($b['email']) ?></td>
											<td><?= htmlspecialchars($b['phonenumber']) ?></td>
											<td><?= date("F d Y", strtotime($b['added_date'])) ?></td>
											<td>
												<div class="user_actions d-flex align-items-center justify-content-center gap-3">
													<a href="add-new-barrower?id=<?= $b['id'] ?>" class=" text-decoration-none text-info">
														<i class="ri-edit-box-line fs-4"></i>
													</a>
													<a href="#" type="button" class="btn_delete_user text-decoration-none text-danger" data-id="<?= $b['id'] ?>" data-role="<?= $b['role'] ?>">
